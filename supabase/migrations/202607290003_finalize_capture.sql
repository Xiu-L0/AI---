...
add column result_source_version_id uuid;

alter table public.capture_sessions
add constraint capture_sessions_result_version_item_owner_fk
foreign key (
...
create index capture_sessions_result_version_idx
on public.capture_sessions (result_source_version_id)
where result_source_version_id is not null;
...
begin
  ...
  if v_capture_session.status = 'finalized' then
    ...
    where job.source_version_id = v_existing_version.id
      and job.job_type = 'prepare_for_milestone_b';
