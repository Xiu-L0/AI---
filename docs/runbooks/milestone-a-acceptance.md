...
| 23 | 页面资源权限 / 带附件断网重启恢复 | Edge 含图会话先返回 durable partial；Web 离线时点击一次补充截图，PNG 恢复任务进入待处理，关闭并重开 Edge 后仍保留 | 是；6 条文字消息已经 durable 保存，图片缺失被明确列出 | ChatGPT 图片直接下载仍为 HTTP 403；随后验收主动暂停 Recall Web | 重开 Edge 后恢复 Web并重试；最终 complete receipt 为 6 条消息、1 个附件、异常 0，数据库最新版本缺失项为空且恰好 1 张 PNG | 否；附件 Blob 跨浏览器重启和恢复上传符合设计；“对象已上传但 finalize 中断”的 retry token 场景仍需独立验证 |
...
| 小红书现场检查 | 未实现平台 / 状态语义歧义 | 小红书帖子页明确显示“当前页面暂不支持自动采集”，但弹窗下方同时继续展示上一条 ChatGPT 的“完整采集成功、2 条消息”回执，容易被误解为当前小红书帖子已保存 | 否；当前小红书帖子没有进入采集协议 | Milestone A 暂用 Web 粘贴文字、文件或截图；后续界面应把历史回执明确标为“上一次采集结果”或在不支持页面隐藏，不能让它看起来属于当前页面 | 状态语义修复属于可靠采集诚实反馈；小红书结构化正文与图片适配不属于 Milestone A，应按设计在后续里程碑实现 |
...
| Codex 验证摘要 | 含图会话的静默遗漏、布局过滤和截图恢复状态缺陷均已保留失败证据并修复。第 21 次 Edge 自动完整保存；第 22 次验证无附件 Outbox 跨 Edge 重启；第 23 次验证 partial 的补充截图 PNG 在 Web 离线及关闭/重开 Edge 后仍保留；C-05 又验证凭据撤销后进入 auth_paused、数据不丢失，重新配对后原任务恢复，最新版本 14 为 complete、缺失项为空、6 条消息和 1 张 PNG。此前扩展 151 项测试、类型检查、正式构建、Manifest 与 secret 检查通过 |
| 未解决问题 | 当前 23/30；按现有 `N_auto_complete / N_supported` 口径为 16/23，达到正式 90% 至少需要继续增加 47 次全部自动完整的支持样本（总计 70 次）；Edge 已完成加载、配对、完整 ChatGPT 会话、partial、无附件和带 PNG 的 Outbox 跨浏览器重启及凭据撤销/重新配对；对象已上传但 finalize 中断的附件 retry token、补截图失败的真实浏览器路径以及不支持页面误显上一条成功回执，由 Codex 后续自动验证或修复，不再要求用户重复手工故障注入；小红书结构化适配属于后续里程碑；为保护本地数据，清空式数据库 reset 未执行 |
