...
- [x] 项目状态、稳定 idempotency key 和附件 Blob 在关闭弹窗后仍存在。
- [x] 浏览器工具栏角标显示未解决数量。
- [x] 重新打开浏览器后项目仍存在。
- 结果：`PASS`
- 证据/备注：2026-08-02 选中文字恢复采集期间，本地 Recall Web 服务停止，等价于扩展无法访问 Recall API。扩展明确显示“服务器尚未确认保存”、`Failed to fetch` 和下次自动重试时间，未显示成功，并在待处理采集列表保留项目。2026-08-09 在 Edge 中停止 Recall Web 后出现待处理异常 2，工具栏角标同步显示 2；关闭所有 Edge 窗口并重新打开后，两项仍存在。随后在真实 partial 上生成补充截图，Web 离线时 PNG 恢复任务进入本地待处理队列；关闭全部 Edge 窗口并重开后，截图恢复任务、原 partial 和角标 2 均保留。服务恢复后取得 complete receipt、异常归零；数据库最新版本为 complete、缺失项为空、6 条唯一消息和恰好 1 张 image/png。由此验证文本元数据、稳定恢复关系和附件 Blob 均可跨浏览器重启。
...
- [x] 在 `retry_wait` 项上点击“立即重试”。
- [ ] 重试期间仍不显示成功。
- [x] 成功后显示服务器真实保存数量。
- [ ] 失败后更新可理解的原因和下一次重试时间，项目不消失。
- 结果：`NOT RUN`
- 证据/备注：2026-08-09 Edge 截图恢复任务在 Web 离线时进入 `retry_wait`；Web 恢复后只触发一次重试，最终显示 complete receipt、6 条消息、1 个附件、异常 0。数据库核对最新版本恰好 6 条唯一消息和 1 张 PNG。由于没有单独截取“重试进行中”画面，也未覆盖重试再次失败后的原因更新时间，C-03 暂不整体标记 PASS。
...
- [x] 在 Web 设置页撤销当前扩展设备。
- [x] 下一次网络任务进入 `auth_paused`，未解决数据不被清除。
- [x] 重新配对后暂停任务重新安排执行。
- [x] 恢复任务仍使用原稳定 idempotency key。
- 结果：`PASS`
- 证据/备注：2026-08-09 在 Web 设置页主动撤销 Edge 扩展设备后，再次提交真实 ChatGPT 含图会话。扩展显示待处理异常 1、“扩展连接已失效，请重新配对”和 `Extension authentication expired`，没有显示成功，原待处理项目仍可见。使用新的一次性配对码重新连接后，没有再次点击“保存到 Recall AI”，原项目恢复并取得 complete receipt：6 条消息、1 个附件、异常 0。数据库仍为同一 SourceItem，新增且仅新增版本 14；最新版本 complete、缺失项为空、6 条消息和 1 张 PNG。该主动撤销凭据场景不满足正式分母所需的有效扩展授权前提，因此作为 C-05 负向恢复测试记录，不计入 30 次真实支持样本。
...
| partial 缺失项显示 | PASS | PASS | Chrome 与 Edge 的真实含图会话均在图片资源 HTTP 403 时显示具体缺失项；Edge 补充截图在断网和重启后恢复为 6 条消息、1 张 PNG、异常归零 |
...
| 关闭并重开浏览器后 Outbox 仍在 | NOT RUN | PASS | Edge 已分别验证无附件文本任务和带 PNG 的截图恢复任务；关闭全部窗口并重开后，工具栏角标、待处理元数据和附件 Blob 引用均保留 |
| 撤销凭据后重新配对恢复 | NOT RUN | PASS | Edge 撤销设备后任务进入 auth_paused 且数据保留；重新配对后原项目恢复为 6 条消息、1 张 PNG、异常 0，同一 SourceItem 仅新增版本 14 |
...
| 23 | 2026-08-09 | Edge | ChatGPT 含一张合成图片的完整会话 / partial 后截图兜底断网重启 | 是 | 自动部分 | 是（先取得 durable partial receipt，明确列出图片 HTTP 403） | Web 离线时补充截图；关闭并重开 Edge；恢复 Web 后重试 | 人工兜底后完整保存 | 原文先以 6 条消息 durable 保存并列出第 5 条图片 HTTP 403；补充截图 PNG 在 Web 离线时进入本地待处理队列，重开 Edge 后仍存在。恢复后 complete receipt 为 6 条消息、1 个附件、异常 0。数据库最新版本 13 为 complete、缺失项为空、6 条唯一消息和恰好 1 张 image/png。 |
| 24 |  |  |  |  |  |  |  |  |  |
| 25 |  |  |  |  |  |  |  |  |  |
| 26 |  |  |  |  |  |  |  |  |  |
| 27 |  |  |  |  |  |  |  |  |  |
...
16 / 23 × 100% = 69.57%（阶段值，样本不足且尚未达到正式目标）
```
...
历史累计指标继续按上一节的 `N_supported = 23`、`N_auto_complete = 16`、`69.57%` 记录，不得改分母或删除旧样本。按该历史口径达到累计 90%，至少还需要 47 次全部自动完整的支持样本。

Stage 0 之后另开一个向前的发布候选窗口，不替代历史分母：

| 发布候选要求 | 当前记录 |
| --- | --- |
| Stage 0 修复后连续 30 次符合支持条件的真实采集 | 0 / 30，尚未开始 |
| 至少 27 次首次自动 complete | 0 |
| 错误完整或提前成功 | 0 |
| 每个 partial/failure 都有分类原因和恢复记录 | 窗口尚未开始 |
| Chrome 与 Edge 均有代表样本 | 窗口尚未开始 |

该窗口只有在真实浏览器连续样本满足上表后才能签署通过；自动化回归不能把分母改写成已通过。
...
| 22 | 服务 / 浏览器重启恢复 | Recall Web 停止时 Edge 显示待处理异常 2；关闭全部 Edge 窗口并重开后项目仍保留，没有显示成功 | 否 | 验收主动暂停本地 Web 服务；数据库仍在线 | 重开 Edge 后恢复 Web，原项目重试取得 complete receipt；数据库只有 1 个 SourceItem、1 个 complete 版本和 2 个唯一消息 ID | 否；无附件的浏览器重启持久性与服务恢复去重符合设计；附件 Blob 由第 23 次独立验证 |
| 23 | 页面资源权限 / 带附件断网重启恢复 | Edge 含图会话先返回 durable partial；Web 离线时点击一次补充截图，PNG 恢复任务进入待处理，关闭并重开 Edge 后仍保留 | 是；6 条文字消息已经 durable 保存，图片缺失被明确列出 | ChatGPT 图片直接下载仍为 HTTP 403；随后验收主动暂停 Recall Web | 重开 Edge 后恢复 Web并重试；最终 complete receipt 为 6 条消息、1 个附件、异常 0，数据库最新版本缺失项为空且恰好 1 张 PNG | 否；附件 Blob 跨浏览器重启和恢复上传符合设计。上传完成但 finalize 中断的同幂等键恢复已由 `capture-runner` 双轮测试覆盖，fresh signed target 不写入 Outbox |
| C-05 负向检查 | 凭据撤销 / 重新配对恢复 | Web 主动撤销 Edge 设备后，下一次提交显示待处理异常 1、“扩展连接已失效”和 `Extension authentication expired`，没有显示成功 | 否 | 验收主动撤销扩展 token；此时不满足正式支持样本的有效授权前提 | 使用新一次性配对码重新连接，不再次保存；原项目恢复为 complete receipt，6 条消息、1 张 PNG、异常 0，同一 SourceItem 仅新增版本 14 | 否；auth_paused、数据保留、重新配对和稳定任务恢复符合设计，不计入正式样本分母 |
| 补截图失败边界 | 恢复失败 / 诚实保留 | 截图 API 拒绝或本地附件存储失败时，不得删除原 partial，也不得伪造失败服务回执 | 是；原 partial receipt 必须保留 | 补充截图发生在 durable partial 之后，失败点在 Blob 创建或本地存储 | 原项保持 unresolved；单飞在 finally 中清除；稍后重试只创建一条恢复子项 | 是；Stage 0 已用 background-controller 与弹窗回归覆盖两条失败边界 |
| 小红书现场检查 | 未实现平台 / 状态语义已修复 | 小红书帖子页明确显示“当前页面暂不支持自动采集”，历史现场同时继续展示上一条 ChatGPT 的“完整采集成功”回执 | 否；当前小红书帖子没有进入采集协议 | 不支持页面曾把历史 complete 回执渲染成当前页结果 | 弹窗改为仅在当前页受支持或用户选中未解决任务时展示历史回执；不支持页显示“当前页面未采集；可改用 Web 应用保存。” | 是；Stage 0 已用弹窗回归测试关闭该状态语义。小红书结构化正文与图片适配仍属后续里程碑 |
...
| Gate | 结果 | 说明 |
| --- | --- | --- |
| 自动化测试与构建 | PASS | lint、typecheck、35 个测试文件、272 项单元测试、build 通过。pgTAP 6 个文件、80 项断言通过。本机 Windows 排除了文档端口 54321–54324，E2E 无法连上本地邮件入口，不改历史 12 项 E2E 结论 |
...
| 最终结论 | CONDITIONAL |
| 日期 | 2026-09-06 |
| 用户确认 | Chrome 与 Edge 真实验收进行中；第 17–23 次、第 19 次截图兜底、第 20 次服务恢复、第 22 次文本重启恢复和第 23 次带 PNG 重启恢复结果由用户截图确认 |
| Codex 验证摘要 | 含图会话的静默遗漏、布局过滤和截图恢复状态缺陷均已保留失败证据并修复。第 21 次 Edge 自动完整保存；第 22 次验证无附件 Outbox 跨 Edge 重启；第 23 次验证 partial 的补充截图 PNG 在 Web 离线及关闭/重开 Edge 后仍保留；C-05 又验证凭据撤销后进入 auth_paused、数据不丢失，重新配对后原任务恢复。Stage 0 另增：不支持页面不再把历史 complete 显示为当前结果；上传完成但 finalize 中断后可用同一幂等键和 fresh signed target 恢复；截图采集或本地附件存储失败后原 partial 与“补充截图”入口保持可恢复。已验证备份 `2026-09-06T04:51:43.921Z` 通过清单校验。lint、typecheck、272 项单元测试、build 与 80 项 pgTAP 通过。历史真实验收仍为 16/23 |
| 未解决问题 | 历史累计仍为 23/30、16/23；发布候选窗口 0/30，正式 90% 仍未签署。小红书结构化适配属于后续里程碑。为保护本地数据，清空式数据库 reset 未执行 |
